(function () {
  ...
  async function preloadNextEpisodeParts(customStartIndex = null) {
    ...
    if (isPreloadingInProgress) {
      if (PLAYER_CONFIG.debugMode)
        console.log('[Preload] Aborted: another preload is already in progress.');
      return;
    }
    ...
    if (isSlowNetwork()) {
      if (PLAYER_CONFIG.debugMode)
        console.log('[Preload] Skipping preloading due to slow network.');
      return;
    }
    ...
    try {
      ...
      if (PLAYER_CONFIG.debugMode) {
        console.log(
          `[Preload] Start index: ${startIndex}, Current index: ${currentIndex}, Total: ${totalEpisodes}, Count: ${preloadCount}`
        );
      }
      ...
      for (
        let offset = 1, loaded = 0, tried = 0;
        loaded < preloadCount && tried < maxTryLimit;
        offset++, tried++
      ) {
        // ① 计算要尝试的剧集索引
        ...
        // ③ 如果已经预加载过 → 直接跳过
        if (preloadedEpisodeUrls.has(nextEpisodeUrl) || inFlightEpisodeUrls.has(nextEpisodeUrl)) {
          if (PLAYER_CONFIG.debugMode)
        ...
        try {
          ...
          try {
            ...
            if (PLAYER_CONFIG.debugMode)
              console.log(`[Preload] Direct fetch failed for ${nextEpisodeUrl}, trying proxy...`);
            // 如果直接抓取失败（可能是CORS），尝试通过代理抓取
            const proxyUrl =
              typeof PROXY_URL !== 'undefined'
                ? PROXY_URL + encodeURIComponent(nextEpisodeUrl)
                : null;
            if (proxyUrl) {
          ...
          if (!m3u8Response || !m3u8Response.ok) {
            if (PLAYER_CONFIG.debugMode)
              console.log(`[Preload] Failed to fetch m3u8 for ep ${episodeIdxToPreload + 1}`);
            continue;
          }

          const m3u8Text = await m3u8Response.text();
          const tsUrls = [];
          const baseUrlForSegments = nextEpisodeUrl.substring(
            0,
          ...
          for (let i = 0; i < lines.length && tsUrls.length < 3; i++) {
            ...
            if (line && !line.startsWith('#')) {
              ...
              let fullTsUrl = line.startsWith('http')
                ? line
                : new URL(line, baseUrlForSegments).href;
          ...
          for (const tsUrl of tsUrls) {
            ...
            try {
              ...
              if (PLAYER_CONFIG.debugMode)
                console.warn(`[Preload] TS fetch failed: ${tsUrl}`, tsErr);
            }
          }
  ...
  function registerPreloadEvents() {
    if (!window.player) {
      if (PLAYER_CONFIG.debugMode)
        console.log('[Preload] Player not ready, deferring event registration.');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    setTimeout(() => {
      ...
      if (isEnabled) {
        ...
        if (PLAYER_CONFIG.debugMode)
          console.log('[Preload] Preloading is disabled by user setting on page load.');
      }
    }, 500);
  });
