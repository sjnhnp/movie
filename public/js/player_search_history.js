...
function setupPlayerSearchHistory() {
  ...
  if (typeof AppState !== 'undefined' && !AppState.get('selectedAPIs')) {
    ...
    const selectedAPIs = selectedAPIsRaw
      ? JSON.parse(selectedAPIsRaw)
...
async function performPlayerSearch(query) {
  ...
  try {
    ...
    if (
      ...
      results.some((item) => !item.loadSpeed || item.loadSpeed === '检测中...')
    ) {
      // 延迟触发速度检测，确保DOM已渲染
      setTimeout(() => {
        if (typeof backgroundSpeedUpdate === 'function') {
          backgroundSpeedUpdate(results)
            .then(() => {
              // 重新排序并刷新速度标签
...
function createResultItemUsingTemplate(item) {
  ...
  if (speedElement) {
    if (item.loadSpeed && isValidSpeedValue(item.loadSpeed)) {
      ...
      // 设置速度标签的颜色
      speedElement.className =
        'text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-green-500 text-green-300';
    } else if (item.loadSpeed === '检测中...') {
      speedElement.textContent = '检测中...';
      speedElement.classList.remove('hidden');
      speedElement.className =
        'text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-yellow-500 text-yellow-300';
    } else {
      // 如果没有速度信息，隐藏标签
...
function showPlayerVideoDetail(item) {
  ...
  const fields = {
    ...
    area: item.vod_area || '未知',
    remarks: item.vod_remarks || '无',
    director: item.vod_director || '未知',
    actor: item.vod_actor || '未知',
    description: (item.vod_content || item.vod_blurb || '暂无简介').replace(/<[^>]+>/g, '').trim(),
  };
  ...
  if (episodesContainer && item.vod_play_url) {
    ...
    if (item.vod_play_url.includes('$$$')) {
      ...
      // 优先选择包含当前播放源名字的组，或者选第一组
      episodes = groups[0].split('#').filter((ep) => ep.trim());
    } else {
      episodes = item.vod_play_url.split('#').filter((ep) => ep.trim());
    }
    ...
    if (episodeButtonsGrid) {
      ...
      const isVarietyShow = varietyShowTypes.some(
        (type) => item.type_name && item.type_name.includes(type)
      );
      ...
      episodes.forEach((episode, index) => {
        ...
        btn.className = isVarietyShow
          ? 'episode-btn'
          : 'px-3 py-2 bg-white/5 hover:bg-indigo-500/80 text-white/90 rounded-xl text-sm transition-all duration-300 border border-white/10 hover:border-indigo-400 shadow-lg truncate';
        ...
        btn.addEventListener('click', () => {
          ...
          playerUrl.searchParams.set('url', epUrl);
...
function initializeQualityTag(element, item) {
  ...
  element.className =
    'quality-tag text-xs font-medium py-0.5 px-1.5 rounded bg-gray-500 text-white cursor-pointer transition-colors hover:opacity-80 whitespace-nowrap flex-shrink-0';

  // 异步检测画质
  detectVideoQuality(item)
    .then((quality) => {
...
function handleQualityTagClick(element, item) {
  ...
  if (currentQuality === '未知' || currentQuality === '检测失败' || currentQuality === '检测超时') {
    ...
    element.textContent = '检测中...';
    element.className =
      'quality-tag text-xs font-medium py-0.5 px-1.5 rounded bg-gray-500 text-white cursor-pointer transition-colors hover:opacity-80';

    // 执行画质重测
    detectVideoQuality(item, true)
      .then((quality) => {
        updateQualityTag(element, quality);
        item.quality = quality;
        // 清除防抖记录
