...
async function playVideo(
  ...
  const isSpecialSource =
    !sourceCode.startsWith('custom_') && API_SITES[sourceCode] && API_SITES[sourceCode].detail;
  if (isSpecialSource) {
...
function generateUniversalId(title, year, episodeIndex) {
  // 提取核心标题和归一化
  const normalizedTitle = getCoreTitle(title)
    .toLowerCase()
    .replace(/[^\w\u4e00-\u9fa5]/g, '')
    .replace(/\s+/g, '');
...
function initializeEventListeners() {
  ...
  if (adFilteringToggle) {
    ...
    adFilteringToggle.checked = getBoolConfig(
      PLAYER_CONFIG.adFilteringStorage,
      PLAYER_CONFIG.adFilteringEnabled
    );
  }
  ...
  if (speedDetectionToggle) {
    ...
    speedDetectionToggle.checked = getBoolConfig(
      PLAYER_CONFIG.speedDetectionStorage,
  ...
  performSearch(query, selectedAPIs)
    ...
    .catch((error) => {
      if (searchResultsContainer)
        searchResultsContainer.innerHTML = `<div class="text-center py-4 text-red-400">搜索出错: ${error.message}</div>`;
    })
    .finally(() => {
      if (isNormalSearch && typeof hideLoading === 'function') hideLoading();
      if (typeof options.onComplete === 'function') options.onComplete();
...
function rebuildVideoCaches(results) {
  ...
  localStorage.setItem('videoSourceMap', JSON.stringify(Array.from(videoSourceMap.entries())));
}
    ...
    while (cursor < results.length) {
      ...
      try {
        ...
        if (cachedResultsRaw) {
          ...
          const idx = arr.findIndex((it) => `${it.source_code}_${it.vod_id}` === key);
          if (idx !== -1) {
...
async function performSearch(query, selectedAPIs) {
  ...
  // 检查是否启用速度检测
  const speedDetectionEnabled = getBoolConfig(
    PLAYER_CONFIG.speedDetectionStorage,
    PLAYER_CONFIG.speedDetectionEnabled
  );
  ...
  if (cacheResult.canUseCache) {
    ...
    if (
      cacheResult.results.some((r) => r.detectionMethod === 'pending' || r.quality === '检测中...')
    ) {
  ...
  const searchPromises = selectedAPIs
    .map((apiId) => {
      ...
      return fetch(apiUrl)
        ...
        .then((data) => ({
          ...data,
          apiId: apiId,
          apiName: APISourceManager.getSelectedApi(apiId)?.name || apiId,
        }))
        .catch((error) => ({
          code: 400,
        ...
        result.list.forEach((item) => {
          allResults.push({
            ...
            api_url: result.apiId.startsWith('custom_')
              ? APISourceManager.getCustomApiInfo(parseInt(result.apiId.replace('custom_', '')))
                  ?.url
              : '',
          });
        });
...
function renderSearchResults(allResults, doubanSearchedTitle = null) {
  ...
  if (yellowFilterEnabled && allResults && allResults.length > 0) {
    ...
    allResults = allResults.filter((item) => {
      ...
      const isWhitelisted = whitelistKeywords.some(
        (keyword) => title.includes(keyword) || type.includes(keyword)
      );
  ...
  // 预加载处理
  if (
    typeof window.startPreloading !== 'undefined' &&
    typeof window.stopPreloading !== 'undefined'
  ) {
      ...
      if (
        ...
        backgroundSpeedUpdate(parsed).then(() => {
          ...
          try {
            ...
            console.error('回写缓存失败', e);
          }
        });
      }

      /* —— 再补画质（若需要） —— */
      if (parsed.some((r) => r.detectionMethod === 'pending' || r.quality === '检测中...')) {
...
async function getVideoDetail(id, sourceCode, apiUrl = '') {
  ...
  if (searchResults) {
    searchResults.innerHTML =
      '<div class="text-center py-4"><div class="spinner"></div><p class="mt-2 text-gray-400">正在获取视频信息，请稍候...</p></div>';
  }
  ...
  try {
    ...
    playVideo(firstEpisode, data.videoInfo?.title || '未知视频', 0, sourceName, sourceCode, id);
  } catch (error) {
    if (searchResults) {
...
async function showVideoEpisodesModal(id, title, sourceCode, apiUrl, fallbackData) {
  ...
  if (isSpecialSource || isCustomSpecialSource) {
    setTimeout(async () => {
      try {
        ...
        const episodeGrid = document.querySelector(
  ...
  const sourceNameForDisplay =
    videoData.source_name || APISourceManager.getSelectedApi(sourceCode)?.name || '未知源';
  AppState.set('currentEpisodes', episodes);
  ...
  const fields = {
    ...
    description: (videoData.vod_blurb || fallbackData.blurb || '暂无简介。')
      .replace(/<[^>]+>/g, '')
      .trim(),
  ...
  const isVarietyShow = varietyShowTypes.some(
    (type) => effectiveTypeName && effectiveTypeName.includes(type)
  );
  if (episodeButtonsGrid) {
    if (isVarietyShow) {
      episodeButtonsGrid.className = 'variety-grid-layout';
    }
    episodeButtonsGrid.innerHTML = renderEpisodeButtons(
      episodes,
      effectiveTitle,
      sourceCode,
      sourceNameForDisplay,
  ...
  if (orderIcon) {
    orderIcon.style.transform =
      AppState.get('episodesReversed') || false ? 'rotate(180deg)' : 'rotate(0deg)';
  }
...
function toggleEpisodeOrderUI(container) {
  ...
  if (episodes && title && sourceCode) {
    container.innerHTML = renderEpisodeButtons(
      episodes,
      title,
      sourceCode,
      sourceName || '',
...
function renderEpisodeButtons(episodes, videoTitle, sourceCode, sourceName, typeName) {
  ...
  return displayEpisodes
    .map((episodeString, displayIndex) => {
      const originalIndex = currentReversedState
        ? episodes.length - 1 - displayIndex
        : displayIndex;
      const originalEpisodeNames = AppState.get('originalEpisodeNames') || [];
...
function updateQualityBadgeUI(qualityId, quality, badgeElement) {
  const badge =
    badgeElement || document.querySelector(`.quality-badge[data-quality-id="${qualityId}"]`);
  if (!badge) return;
...
async function manualRetryDetection(qualityId, videoData) {
  ...
  badge.className =
    'quality-badge text-xs font-medium py-0.5 px-1.5 rounded bg-gray-500 text-white';
  badge.style.cursor = 'default';
  ...
  try {
    ...
    try {
      ...
      if (searchResultsRaw) {
        ...
        const indexToUpdate = resultsArray.findIndex(
          (item) => `${item.source_code}_${item.vod_id}` === qualityId
        );
