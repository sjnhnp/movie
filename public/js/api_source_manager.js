...
const APISourceManager = {
  ...
  initAPICheckboxes: function () {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS) {
      ...
      Object.keys(API_SITES)
        .filter((key) => API_SITES[key].adult)
        .forEach((apiKey) => {
          const api = API_SITES[apiKey];
          let box = document.createElement('div');
          box.className = 'flex items-center';
  ...
  checkAdultAPIsSelected: function () {
    ...
    const yellowToggle =
      DOMCache.get('yellowFilterToggle') || document.getElementById('yellowFilterToggle');
    ...
    if (hasAdult) {
      ...
      if (desc)
        desc.innerHTML = '<strong class="text-pink-300">选中黄色资源站时无法启用此过滤</strong>';
      // 去除提示信息
  ...
  editCustomApi: function (index) {
    ...
    const isAdultInput =
      DOMCache.get('customApiIsAdult') || document.getElementById('customApiIsAdult');
    const detailInput =
      DOMCache.get('customApiDetail') || document.getElementById('customApiDetail');
  ...
  updateSelectedAPIs: function () {
    const builtIn = Array.from(document.querySelectorAll('#apiCheckboxes input:checked')).map(
      (input) => input.dataset.api
    );
    const custom = Array.from(document.querySelectorAll('#customApisList input:checked')).map(
  ...
  updateSelectedApiCount: function () {
    const countElement =
      DOMCache.get('selectedApiCount') || document.getElementById('selectedApiCount');
    if (countElement) {
