...
async function fetchWithTimeout(targetUrl, options, timeout = 10000, responseType = 'json') {
  ...
  try {
    ...
    clearTimeout(timer);
    if (!response.ok)
      throw new Error(`请求失败: ${response.status} ${response.statusText} for ${targetUrl}`);
    // 根据类型返回结果
      ...
      try {
        ...
        result.list.forEach((item) => {
          item.source_name = source.startsWith('custom_')
            ? window.APISourceManager?.getCustomApiInfo(parseInt(source.replace('custom_', '')))
                ?.name || '自定义源'
          ...
          return JSON.stringify({
            ...
            videoInfo: {
              ...
              source_name: sourceCode.startsWith('custom_')
                ? window.APISourceManager?.getCustomApiInfo(
                    parseInt(sourceCode.replace('custom_', ''))
                  )?.name || '自定义源'
                : API_SITES && API_SITES[sourceCode]
