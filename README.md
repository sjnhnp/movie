...
基于 [**LibreSpark/LibreTV**](https://github.com/LibreSpark/LibreTV) 的深度优化重构，保持最原初作为播放器的纯粹。
...
* **历史记录**:
    ...
    * **搜索历史**: 由“豆瓣热门”产生的搜索结果不会计入个人搜索历史。
* **设置面板**:
    * **分片广告过滤**: 可选的广告过滤功能，如果开启后有些数据源会卡住，请记得关闭。
    * **播放预加载**：自定义预加载集数+开关。
...
* **无缝线路切换**: 切换不同线路时（即搜索结果数据源），能够保持所有历史的播放进度，实现无缝切换体验。
* **记住进度**:
    * 独立于观看历史，可记住每一集的具体播放进度，跨线路共享播放进度。
...
可一键部署于 Cloudflare Pages/Vercel。基础部署流程请参考 [**上游项目文档**](https://github.com/LibreSpark/LibreTV)。
